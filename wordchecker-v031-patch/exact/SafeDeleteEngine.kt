...
/** High-precision deterministic adjacent duplicate detector. */
class SafeDeleteEngine {
    ...
    fun lastWord(prefix: String): String? =
        wordRegex.findAll(prefix).lastOrNull()?.value?.let(::normalizeWord)
    ...
    fun decide(
        ...
        userProtectedWords: Set<String> = emptySet(),
    ): CorrectionDecision {
        ...
        // The IME normalizes learned repeats once when preferences are loaded.
        // Avoid allocating a new HashSet on every word boundary.
        if (n2 in userProtectedWords) {
            return CorrectionDecision(false, "user_learned_repeat", confidence = 1.0, word = secondWord)
        }
        if (n2 in protectedWords) {
            return CorrectionDecision(false, "known_intentional_or_grammatical_repeat", confidence = 0.55, word = secondWord)
        }

        val beforeFirst = prefix.substring(0, first.range.first)
        if (n2 in contextual && whClauseRegex.containsMatchIn(beforeFirst)) {
            return CorrectionDecision(false, "contextual_grammatical_repeat", confidence = 0.60, word = secondWord)
        }
